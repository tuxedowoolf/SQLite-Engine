	...
	for (int x = columnStart; x < noTokens - 1; x++)
	{
		...
		if (strcmp(colTokens[1], "TEXT") == 0)
		{
			if (colTokens[2][0] == '\"')
			{
				...
				textTokens[0] = colTokens[2];
				int noText = 1;
				int y = x;
				int t = 1;

				int textSize = strlen(colTokens[2]);
				...
				Column column(columnName, columnType, defaultText);
		...
		{
			columnType = INTEGER;
			defaultInt = atoi(colTokens[2]);
			Column column(columnName, columnType, defaultInt);
			columns[x - columnStart] = column;
		}
		else if (strcmp(colTokens[1], "FLOAT") == 0)
		{
			columnType = FLOAT;
			defaultFloat = atof(colTokens[2]);
			Column column(columnName, columnType, defaultFloat);
			columns[x - columnStart] = column;
		}
		else
		{
